...
//	    MPI_Scatter(t->scatter_sendvec_d, t->num_of_pop_per_spawn * t->N / 2, MPI_DOUBLE, t->scatter_rcvvec_d, t->num_of_pop_per_spawn * t->N / 2, MPI_DOUBLE, 0, t->spawn_comm); // in main and make_neuro_spawn
        ...
        printf("weight vector (@parent proc)\n");
	    int l;
        for(l=0; l<t->N * (t->lambda + t->num_of_pop_per_spawn); ++l){
            if (l%t->N == 0){
                printf("--------------Gene [%d]-----\n%f\t", l/t->N, t->scatter_sendvec[l]);
            }else if (l%t->N == t->N-1){
                printf("%f\n", t->scatter_sendvec[l]);
            }else{
                printf("%f\t", t->scatter_sendvec[l]);
            }
        }
        fflush(stdout);
        ...
        // update prev arfitness for all gene
	    for(i=0; i<t->lambda; i++)
